...
class ASRTimeAligner:
    ...
    def timeAlign(self, audioPath: str, transcript_name:str):
        """
        ...
        for index, row in dair_csv.iterrows():
            ...
            # punctuate, and get ner for this sentence.
            transcript = transcript.lower()
            punc_restore = self.punctuate(transcript)
            namedEntity = ner(punc_restore)
            total_conf += confidence
            transcriptions.append({"Start (sec.)": str(start_t),
                                   "End (sec.)": str(end_t),
                                   "Speaker": str(row['ID']),
                                   "Transcript": str(punc_restore),
